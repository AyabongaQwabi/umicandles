...
export class ShiplogicService {
  ...
  constructor() {
    ...
    this.accountId = process.env.SHIPLOGIC_ACCOUNT_ID
      ? Number.parseInt(process.env.SHIPLOGIC_ACCOUNT_ID)
      : undefined;

    if (!this.apiKey) {
      console.warn(
        'Shiplogic API key not found. Set SHIPLOGIC_API_KEY environment variable.'
      );
    }

    if (!this.accountId) {
      console.warn(
        'Shiplogic Account ID not found. Set SHIPLOGIC_ACCOUNT_ID environment variable.'
      );
    }
  }
  ...
  private async fetchWithAuth(endpoint: string, options: RequestInit = {}) {
    ...
    // Use Bearer token authentication as per Shiplogic documentation
    const headers = {
  ...
  async getRates(
    request: ShiplogicRateRequestV2
  ): Promise<ShiplogicRatesResponseV2> {
    try {
      // Validate the request
      console.log('Validating rate request:', JSON.stringify(request, null, 2));
      this.validateRateRequest(request);
      console.log('Rate request is valid');

      // Add account_id if available
      if (this.accountId) {
        request.account_id = this.accountId;
      }
      console.log('Sending rate request:', JSON.stringify(request, null, 2));
      // Use the correct v2 endpoint
      return await this.fetchWithAuth('/v2/rates', {
        method: 'POST',
  ...
  private validateRateRequest(request: ShiplogicRateRequestV2) {
    ...
    request.parcels.forEach((parcel, index) => {
      if (!parcel.submitted_length_cm || parcel.submitted_length_cm <= 0) {
        throw new Error(`Parcel ${index + 1} length must be greater than 0`);
      }

      if (!parcel.submitted_width_cm || parcel.submitted_width_cm <= 0) {
        throw new Error(`Parcel ${index + 1} width must be greater than 0`);
      }

      if (!parcel.submitted_height_cm || parcel.submitted_height_cm <= 0) {
        throw new Error(`Parcel ${index + 1} height must be greater than 0`);
      }

      if (!parcel.submitted_weight_kg || parcel.submitted_weight_kg <= 0) {
        throw new Error(`Parcel ${index + 1} weight must be greater than 0`);
      }
    });
  }
  ...
  async createShipment(
    request: ShiplogicShipmentRequestV2
  ): Promise<ShiplogicShipmentResponseV2> {
    // Add account_id if available
    if (this.accountId) {
      request.account_id = this.accountId;
    }

    return await this.fetchWithAuth('/v2/shipments', {
      method: 'POST',
      body: JSON.stringify(request),
    });
  }
  ...
  async getShipment(shipmentId: number): Promise<ShiplogicShipmentResponseV2> {
    return await this.fetchWithAuth(`/v2/shipments/${shipmentId}`);
  }
  ...
  async trackShipment(
    ...
    return await this.fetchWithAuth(
      `/v2/tracking/shipments?tracking_reference=${trackingReference}`
    );
  }
  ...
  createRateRequestFromOrder(order: any): ShiplogicRateRequestV2 {
    ...
    const request: ShiplogicRateRequestV2 = {
